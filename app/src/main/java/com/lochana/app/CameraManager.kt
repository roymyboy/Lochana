...
class CameraManager(
        ...
        try {
            ...
            cameraProviderFuture.addListener({
                ...
                try {
                    ...
                    // Initialize Extensions Manager (may fail on some devices - non-critical)
                    try {
                        initializeExtensions(cameraProvider)
                    } catch (e: Exception) {
                        Log.w(TAG, "⚠️ Extensions initialization failed (continuing without): ${e.message}")
                    }
    ...
    private fun bindCameraUseCases(cameraProvider: ProcessCameraProvider) {
        try {
            ...
            Log.d(TAG, "📷 Creating Preview use case - AspectRatio: ${if (aspectRatio == androidx.camera.core.AspectRatio.RATIO_4_3) "4:3" else "16:9"}")
